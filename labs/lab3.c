...
typedef struct so
{
    ...
    double m[4][4]; 
    double u0; // u lower bound
    double uf; // u upper bound
    double v0; // v lower bound
    double vf;
} scene_object;
...
void add_to_scene(
    ...
    //add to scene
    scene_object* toAdd = &SCENE[SCENE_N];
    toAdd->func = func; 
    memcpy(&toAdd->m, &m, (sizeof(double[4][4])));
    toAdd->u0 = u0;
    toAdd->uf = uf;
    toAdd->v0 = v0;
    toAdd->vf = vf;
...
void sphere_at(double p[3], double r) {
    ...
    add_to_scene(sphere, m, 0, M_PI*2.0, -M_PI/2.0, M_PI/2.0);
}
...
void hyp_base(double p[3], double length, double rz, double ry) {
    ...
    add_to_scene(hyperboloid, m, -1, 1, 0, M_PI*2.0);
}
...
// Scene construction and rendering isn't seperated here because
// the assumption is that we'll be using out-of-process 
// parallelization
void render(int frame_number) {
    // ################################
    // 0. Black background
    // ################################
    SCENE_N = 0; //reset the scene
    set_color(0, 0, 0);
    G_clear();
